...
src = '/home/tanghm/Documents/YFF/R2CNN_FPN_Tensorflow-master/data/VOCdevkit_train/JPEGImages2/'
desrc = '/home/tanghm/Documents/YFF/R2CNN_FPN_Tensorflow-master/data/VOCdevkit_train/JPEGImages/'
jsonList = [x for x in os.listdir(src)]
for jsonFile in tqdm(jsonList):
    with open(src+jsonFile,'r') as f:
        #jsonData = json.loads(f.read())
        #print(src+jsonFile[:-4]+'jpg')
        imgOri = cv2.imread(src+jsonFile)
        w,h,_ = imgOri.shape
        if(w>2000):
...
#             print(imgNew.shape)
        cv2.imwrite(desrc+jsonFile,imgNew)
